...
<section class="card"><h2>{{ __('messages.history') }}</h2>@foreach($form->versions->sortByDesc('version_number') as $version)<div class="list-row"><span>v{{ $version->version_number }} · {{ __('messages.questionnaire_status_'.$version->status) }} @if($version->published_at)· {{ $version->published_at }}@endif</span><span class="actions"><form method="POST" action="{{ route('questionnaires.export-file',[$form,$version]) }}">@csrf<button class="btn">{{ __('messages.export_questionnaire_file') }} · v{{ $version->version_number }}</button></form>@if(in_array(app()->environment(),config('questionnaire_packages.write_environments',[]),true))<form method="POST" action="{{ route('questionnaires.export',[$form,$version]) }}">@csrf<button class="btn">{{ __('messages.export_to_git') }} · v{{ $version->version_number }} ({{ __('messages.questionnaire_status_'.$version->status) }})</button></form>@endif @if($version->status==='draft')<form method="POST" action="{{ route('forms.publish',[$form,$version]) }}">@csrf<button class="btn primary">{{ __('messages.publish') }}</button></form>@elseif($version->status==='published'&&!$form->versions->contains('status','draft'))<form method="POST" action="{{ route('forms.new-draft',[$form,$version]) }}">@csrf<button class="btn">{{ __('messages.new_draft') }}</button></form>@endif</span></div>@endforeach</section>
