@extends('layouts.app')
@section('content')
<div class="page-header"><div><h1>{{ $form->name }}</h1><span class="badge">{{ $form->status }}</span></div><div class="actions"><a class="btn" href="{{ route('forms.builder',$form) }}">{{ __('messages.builder') }}</a><a class="btn" href="{{ route('forms.preview',$form) }}">{{ __('messages.preview') }}</a><form method="POST" action="{{ route('forms.duplicate',$form) }}">@csrf<button class="btn">{{ __('messages.duplicate') }}</button></form>@if($form->status!=='archived')<form method="POST" action="{{ route('forms.archive',$form) }}">@csrf<button class="btn danger">{{ __('messages.archive') }}</button></form>@endif</div></div>
