...
<aside class="card">
    ...
    @foreach($version->attachments as $attachment)<div class="list-row"><a href="{{ route('attachments.download',$attachment) }}">{{ $attachment->original_name }}</a><form method="POST" action="{{ route('attachments.destroy',$attachment) }}" data-confirm="{{ __('messages.confirm_delete') }}">@csrf @method('DELETE')<button class="btn danger" style="border-color:#fca5a5;background:#fef2f2;color:#b91c1c;">{{ __('messages.delete') }}</button></form></div>@endforeach
    ...
    <form method="POST" action="{{ route('builder.components.store',$form) }}" class="stack" data-component-form data-registry='@json($registry)'>@csrf
        ...
        <fieldset data-options data-option-manager data-option-manager-key="new-component" data-next-option-index="3"><legend>{{ __('messages.options') }}</legend><div data-option-list>
            @for($i=0;$i<3;$i++)
                <div class="option-editor" data-option-row data-option-kind="existing"><div class="actions"><strong>{{ __('messages.option_number',['number'=>$i+1]) }}</strong><button class="btn danger" type="button" data-option-remove>{{ __('messages.delete') }}</button></div>
            ...
            @endfor
        </div><template data-option-template><div class="option-editor" data-option-row data-option-kind="existing"><div class="actions"><strong>{{ __('messages.new_option') }}</strong><button class="btn danger" type="button" data-option-remove>{{ __('messages.delete') }}</button></div>@include('forms.partials.locale-tabs',['group'=>'new-component-option-__INDEX__','prefix'=>'options[__INDEX__][translations]','translations'=>null,'fields'=>[['name'=>'label','label'=>__('messages.label'),'base'=>null]]])</div></template><button class="btn" type="button" data-option-add>{{ __('messages.add_option') }}</button></fieldset>
        ...
        <label class="check"><input type="checkbox" name="is_required" value="1"> {{ __('messages.required') }}</label><label class="check"><input type="checkbox" name="visible" value="1" checked> {{ __('messages.visible') }}</label><label class="check"><input type="checkbox" name="is_sensitive" value="1"> {{ __('messages.is_sensitive') }}</label><label class="check"><input type="checkbox" name="manual_grading" value="1"> {{ __('messages.manual_grading') }}</label>
...
<section class="builder-section">
    ...
    <form method="POST" action="{{ route('builder.components.update',[$form,$component]) }}" class="stack">@csrf @method('PUT')
        ...
        @if(in_array($component->type,['single_choice','multiple_choice','dropdown']))<fieldset data-option-manager data-next-option-index="1"><legend>{{ __('messages.options') }}</legend><div data-option-list>@foreach($component->options as $option)<div class="option-editor" data-option-row data-option-kind="existing"><div class="actions"><strong>{{ __('messages.option_number',['number'=>$loop->iteration]) }}</strong><button class="btn danger" type="button" data-option-remove>{{ __('messages.delete') }}</button></div>@include('forms.partials.locale-tabs',['group'=>'option-'.$option->id,'prefix'=>'options[existing]['.$option->id.'][translations]','translations'=>$option->translations,'fields'=>[['name'=>'label','label'=>__('messages.label'),'base'=>$option->label,'required'=>true]]])</div>@endforeach<div class="option-editor" data-option-row data-option-kind="creator"><strong>{{ __('messages.new_option') }}</strong>@include('forms.partials.locale-tabs',['group'=>'new-option-'.$component->id,'prefix'=>'options[new][0][translations]','translations'=>null,'fields'=>[['name'=>'label','label'=>__('messages.label'),'base'=>null]]])</div></div><template data-option-template><div class="option-editor" data-option-row data-option-kind="new"><div class="actions"><strong>{{ __('messages.new_option') }}</strong><button class="btn danger" type="button" data-option-remove>{{ __('messages.delete') }}</button></div>@include('forms.partials.locale-tabs',['group'=>'new-option-'.$component->id.'-__INDEX__','prefix'=>'options[new][__INDEX__][translations]','translations'=>null,'fields'=>[['name'=>'label','label'=>__('messages.label'),'base'=>null]]])</div></template><button class="btn" type="button" data-option-add>{{ __('messages.add_option') }}</button></fieldset>@endif
