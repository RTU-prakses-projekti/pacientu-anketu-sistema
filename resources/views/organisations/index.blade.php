...
<div class="table-wrap"><table><thead><tr><th>{{ __('messages.name') }}</th><th>{{ __('messages.members') }}</th><th>{{ __('messages.forms') }}</th><th>{{ __('messages.actions') }}</th></tr></thead><tbody>
@foreach($organisations as $organisation)
    <tr><td>{{ $organisation->name }}</td><td>{{ $organisation->memberships_count }}</td><td>{{ $organisation->forms_count }}</td><td>
        <div class="actions">
            ...
            <a href="{{ route('forms.index', $organisation) }}">{{ __('messages.forms') }}</a>
