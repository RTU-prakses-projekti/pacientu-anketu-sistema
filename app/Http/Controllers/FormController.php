...
class FormController extends Controller
{
    ...
    public function update(Request $request,Form $form){$this->authorize('update',$form);$data=$request->validate(['name'=>'required|string|max:255']);$form->update($data);return back()->with('success',__('messages.saved'));}
    ...
    public function destroy(Form $form, CleanupService $cleanup) { $this->authorize('update',$form);$organisation=$form->organisation;$cleanup->deleteForm($form);return redirect()->route('forms.index',$organisation)->with('success',__('messages.form_deleted')); }
}
