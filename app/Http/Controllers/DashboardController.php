...
class DashboardController extends Controller
{
    public function __invoke()
    {
        ...
        return view('dashboard', [
            ...
            'activeSubmissions' => FormSubmission::with('publication','formVersion')->where('user_id', $user->id)->where('status', 'in_progress')->get(),
            'completed' => FormSubmission::with('publication','formVersion')->where('user_id', $user->id)->whereIn('status', ['submitted', 'graded', 'awaiting_grading', 'expired'])->latest()->limit(10)->get(),
        ]);
    }
}
