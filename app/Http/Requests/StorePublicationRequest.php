...
class StorePublicationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'access_mode' => ['required', Rule::in($this->isPatientQuestionnaire() ? ['public', 'access_code', 'invitation'] : ['authenticated', 'public', 'access_code', 'invitation'])], 'access_code' => [Rule::requiredIf(fn () => $this->input('access_mode') === 'access_code'), 'nullable', 'string', 'min:6', 'max:100'],
            'opens_at' => ['nullable', 'date'], 'closes_at' => ['nullable', 'date', 'after:opens_at'], 'attempt_limit' => [Rule::requiredIf(fn () => !$this->isPatientQuestionnaire()), 'integer', 'min:1', 'max:100'],
            'timer_enabled' => ['sometimes', 'boolean'], 'duration_minutes' => ['nullable', 'integer', 'min:1', 'max:1440'],
    ...
    private function isPatientQuestionnaire(): bool
    {
        return $this->route('form')?->preset_key === 'patient_questionnaire';
    }
