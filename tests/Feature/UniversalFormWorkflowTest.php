...
class UniversalFormWorkflowTest extends TestCase
{
    ...
    public function test_publication_configuration_validation_and_archived_activation_guards(): void
    {
        ...
        $this->actingAs($creator)->post(route('publications.store',$form),array_merge($base,['access_mode'=>'access_code']))->assertSessionHasErrors('access_code');$this->actingAs($creator)->post(route('publications.store',$form),array_merge($base,['timer_enabled'=>1]))->assertSessionDoesntHaveErrors('duration_minutes');$this->actingAs($creator)->post(route('publications.store',$form),array_merge($base,['anonymous_allowed'=>1,'identified_required'=>1]))->assertSessionHasErrors('anonymous_allowed');
    ...
    public function test_patient_questionnaire_publication_allows_only_patient_access_modes_and_forces_single_attempt(): void
    {
        ...
        $this->actingAs($creator)->post(route('publications.store',$form),array_merge($common,['name'=>'Public','access_mode'=>'public','anonymous_allowed'=>1,'identified_required'=>1,'autosave_enabled'=>0,'resume_enabled'=>0]))->assertRedirect(route('forms.show',$form));
        ...
        $this->assertSame('public',$public->access_mode);$this->assertSame('access_code',$code->access_mode);$this->assertSame('invitation',$invitation->access_mode);foreach([$public,$code,$invitation] as $publication)$this->assertSame(1,$publication->attempt_limit);$this->assertNull($public->access_code_hash);$this->assertNotNull($code->access_code_hash);$this->assertNull($invitation->access_code_hash);foreach([$public,$code,$invitation] as $publication){$this->assertTrue($publication->identified_required);$this->assertFalse($publication->anonymous_allowed);$this->assertTrue($publication->autosave_enabled);$this->assertTrue($publication->resume_enabled);}
        $this->actingAs($creator)->get(route('forms.show',$form))->assertSee('value="public"',false)->assertSee('value="access_code"',false)->assertSee('value="invitation"',false)->assertDontSee('value="authenticated"',false)->assertDontSee('name="attempt_limit"',false)->assertDontSee('name="identified_required"',false)->assertDontSee('name="anonymous_allowed"',false)->assertDontSee('name="autosave_enabled"',false)->assertDontSee('name="resume_enabled"',false);
    }
